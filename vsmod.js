...
function wait_for_property(obj, property, callback) {
  ...
  let end_property;
  end_property = access_stack
    .reduce((a,c) => a === undefined ? undefined : a[c] === undefined ? undefined : a[c], obj )
...
function init(game_instance, game_config) {
  ...
  wait_for_property(game_instance, "scene.scenes.0", () => {
    ...
    let scenes = Object.fromEntries(game_instance.scene.scenes.map((c,i) => [c.sys.config.key, game_config.scene[i]]))
    ...
    for (let mod of mods) {
      for (let pre_callback in mod.pre) {
        ...
        wait_for_property(obj, callback, () => {
          ...
          let function_parent;
          if (access_stack.length === 1)
            function_parent = obj
          else
            function_parent = access_stack
              .reduce((a,c) => a === undefined ? undefined : a[c] === undefined ? undefined : a[c], obj )
...
function extract_data_maps() {
  ...
  let webapck_mod_lookup = ast
    ...
    .filter(n => n.init.properties.length !== 0)[0]
    .init
    .properties
  ...
  let res = webapck_mod_lookup
    ...
    .body
    .body
    .filter(n => n.type === "VariableDeclaration" && n.kind === "const")
    .filter(n => n.declarations
      .some(n => n.type === "VariableDeclarator" && n.init.type === "ClassExpression")
    )
    .map(n => n.declarations)
    .flat()
    .filter(n => n.init.id === null && n.init.superClass === null)
    .filter(n => n.init.body.body
    ...
    .body
    .body
    .filter(n => n.value.body.body
      .some(n => n.type === "IfStatement")
    )
    .map(n => n.value.body.body
      ...
      .map(n => n.expression.expressions
        .filter(n => n.type === "CallExpression")
